...
export function analyzePanel(panel: PanelStats) {
  ...
  const inferredAttackPercent = nakedAttack > 0
    ? Math.max(0, (panel.propertyAttack / nakedAttack - 1) * 100)
    : 0;
  const mainStatGainPerPointPercent = attributeCoefficient > 0 ? (1 / 1250) / attributeCoefficient * 100 : 0;
  const subStatGainPerPointPercent = attributeCoefficient > 0 ? (1 / 5000) / attributeCoefficient * 100 : 0;
  const baseAttackGainPerPointPercent = panel.baseAttack > 0 ? 1 / panel.baseAttack * 100 : 0;
  const attackEquivalentMainStat = mainStatGainPerPointPercent > 0
    ? baseAttackGainPerPointPercent / mainStatGainPerPointPercent
    : 0;
  const attackEquivalentSubStat = subStatGainPerPointPercent > 0
    ? baseAttackGainPerPointPercent / subStatGainPerPointPercent
    : 0;

  return { attributeCoefficient, effectiveMainStat, nakedAttack, inferredAttackPercent, attackEquivalentMainStat, attackEquivalentSubStat, baseAttackGainPerPointPercent, mainStatGainPerPointPercent, subStatGainPerPointPercent };
}
...
export function calculateSkillDps(
  ...
  const attackMultiplier = 1 + ratio(bonuses.attackPercent + inferredAttackPercent + skill.attackPercent);
  const damageMultiplier = 1 + ratio(panel.damagePercent + bonuses.damagePercent + skill.damagePercent);
  const bossMultiplier = 1 + ratio(panel.bossDamagePercent + bonuses.bossDamagePercent + skill.bossDamagePercent);
  const criticalMultiplier = (Math.max(100, panel.criticalDamagePercent + bonuses.criticalDamagePercent + skill.criticalDamagePercent) + 15) / 100;
  const finalDamageMultiplier = 1 + ratio(panel.finalDamagePercent + bonuses.finalDamagePercent + skill.finalDamagePercent);
  const specialMultiplier = Math.max(1, skill.specialMultiplier);
  const combinedMultiplier = attackMultiplier
    * damageMultiplier
    * bossMultiplier
    * criticalMultiplier
  ...
  const overflowDamage = Math.max(0, beforeDefenseDamage - maxDamage) * ratio(panel.overflowConversionPercent);
