...
import {
  ...
  perfectCoreAdminEnabled,
} from "./perfectCoreAdmin";

const skills = ["a", "b", "c", "d", "e", "f"];

describe("perfect core calculator", () => {
  it("normalizes sub-skill order but preserves the main skill", () => {
    expect(getCoreKey({ mainSkillId: "a", subSkillIds: ["c", "b"] })).toBe(
      "a|b|c",
    );
    expect(getCoreKey({ mainSkillId: "b", subSkillIds: ["a", "c"] })).not.toBe(
      "a|b|c",
    );
  });
  ...
  it("normalizes core order for a combination", () => {
    ...
    expect(getCombinationKey([first, second])).toBe(
      getCombinationKey([second, first]),
    );
  });
  ...
  it("generates valid 4 core 6 skill layouts", () => {
    ...
    for (const result of output.results) {
      ...
      expect(new Set(result.cores.map((core) => core.mainSkillId)).size).toBe(
        4,
      );
      expect(Object.values(result.skillCounts)).toEqual([2, 2, 2, 2, 2, 2]);
      expect(Object.values(result.skillLevels)).toEqual([
        50, 50, 50, 50, 50, 50,
      ]);
    }
  });
  ...
  it("generates 3 core 4.5 layouts with one half skill", () => {
    ...
    for (const result of output.results) {
      ...
      expect(
        Object.values(result.skillCounts).filter((count) => count === 2),
      ).toHaveLength(4);
    }
  });
  ...
  it("recommends the next core only from plans containing all owned cores", () => {
    ...
    expect(
      recommendations.every(
        (item) => getCoreKey(item.core) !== getCoreKey(owned[0]),
      ),
    ).toBe(true);
  ...
  it("seeds ten Night Lord skills with six recommended local icons", () => {
    ...
    expect(
      nightLordSeedSkills.filter((skill) => skill.recommended),
    ).toHaveLength(6);
    expect(
      nightLordSeedSkills.every((skill) =>
        skill.iconUrl?.startsWith("/assets/skills/night-lord/"),
      ),
    ).toBe(true);
  });
  ...
  it("seeds ten Adele skills with the six screenshot recommendations", () => {
    ...
    expect(
      adeleSeedSkills.every((skill) =>
        skill.iconUrl?.startsWith("/assets/skills/adele/"),
      ),
    ).toBe(true);
  });
  ...
  it("seeds eight Bishop skills with the six 4-core recommendations", () => {
    ...
    expect(
      bishopSeedSkills.every((skill) =>
        skill.iconUrl?.startsWith("/assets/skills/bishop/"),
      ),
    ).toBe(true);
  });
  ...
  it("seeds nine Shadower skills with the six 4-core recommendations", () => {
    ...
    expect(
      shadowerSeedSkills.every((skill) =>
        skill.iconUrl?.startsWith("/assets/skills/shadower/"),
      ),
    ).toBe(true);
  });
  ...
  it("seeds concrete professions grouped by the reference categories", () => {
    ...
    expect(
      seedProfessions.some(
        (profession) => profession.id === "adventurer-warrior",
      ),
    ).toBe(false);
    expect(
      seedProfessions.find((profession) => profession.id === "night-lord"),
    ).toMatchObject({ name: "隐士", category: "冒险家" });
    expect(
      seedProfessions.filter((profession) => profession.iconUrl),
    ).toHaveLength(52);
  });
  ...
  it("syncs the reference simulator profession skill catalog locally", () => {
    ...
    expect(
      seedSkills.filter((skill) =>
        referenceSeedProfessionIds.includes(skill.professionId),
      ),
    ).toHaveLength(421);
    expect(
      seedSkills
  ...
  it("marks six image-reference skills for each supported 4-core-6 profession", () => {
    for (const professionId of imageFourCoreSixProfessionIds) {
      expect(
        seedSkills.filter(
          (skill) => skill.professionId === professionId && skill.recommended,
        ),
        professionId,
      ).toHaveLength(6);
    }
