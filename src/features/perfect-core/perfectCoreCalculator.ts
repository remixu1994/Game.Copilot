...
export function recommendNextCores(
  ...
  results
    .filter(
      (result) =>
        ownedKeys.size === 0 ||
        [...ownedKeys].every((key) => result.cores.some((core) => getCoreKey(core) === key)),
    )
    .forEach((result) => {
  ...
  return [...candidates.values()].sort(
    (a, b) => b.planCount - a.planCount || getCoreKey(a.core).localeCompare(getCoreKey(b.core)),
  );
}

function buildTargetCounts(request: PerfectCoreRequest): Record<string, number> | string {
  const required = request.mode === '4CORE_6' ? 6 : request.mode === '4CORE_4' ? 4 : 5;
  ...
  if (request.mode === '3CORE_4_5' && !request.halfSkillId) return '请选择一个半强化技能';
  if (request.halfSkillId && !request.selectedSkillIds.includes(request.halfSkillId))
...
export function calculatePerfectCores(request: PerfectCoreRequest): CoreCalculationOutput {
  const targetCounts = buildTargetCounts(request);
  if (typeof targetCounts === 'string') return { results: [], error: targetCounts };
  ...
  function search(
    ...
    if (selected.length === coreCount) {
      ...
      const skillCounts = Object.fromEntries(skills.map((id) => [id, target[id]]));
      results.push({
        cores: [...selected].sort((a, b) => getCoreKey(a).localeCompare(getCoreKey(b))),
        skillCounts,
        // 4核4会让每个技能出现 3 次，但有效强化仍按当前工具规则封顶 50 级。
        skillLevels: Object.fromEntries(skills.map((id) => [id, Math.min(target[id] * 25, 50)])),
      });
      return;
    }
