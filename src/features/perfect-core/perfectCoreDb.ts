...
function seed(db: Database) {
  const professionStatement = db.prepare(
    'INSERT INTO professions (id, name, category, icon_url, sort_order, active) VALUES (?, ?, ?, ?, ?, ?)',
  );
  seedProfessions.forEach((item) =>
    professionStatement.run([
      item.id,
      item.name,
      item.category,
      item.iconUrl ?? '',
  ...
  const skillStatement = db.prepare(
    'INSERT INTO skills (id, profession_id, name, icon_url, sort_order, active, recommended) VALUES (?, ?, ?, ?, ?, ?, ?)',
  );
  seedSkills.forEach((item) =>
    skillStatement.run([
      item.id,
      item.professionId,
      item.name,
  ...
  skillStatement.free();
  db.run(
    "INSERT OR REPLACE INTO app_meta (key, value) VALUES ('seed_version', '3'); INSERT OR REPLACE INTO app_meta (key, value) VALUES ('profession_catalog_version', '3')",
  );
}

function migrateProfessionCatalog(db: Database): boolean {
  const currentVersion = db.exec(
    "SELECT value FROM app_meta WHERE key = 'profession_catalog_version'",
  )[0]?.values[0]?.[0];
  ...
  deleteSkills.run(obsoleteIds);
  deleteSkills.free();
  const deleteProfessions = db.prepare(`DELETE FROM professions WHERE id IN (${placeholders})`);
  deleteProfessions.run(obsoleteIds);
  deleteProfessions.free();
  const statement = db.prepare(
    'INSERT OR REPLACE INTO professions (id, name, category, icon_url, sort_order, active) VALUES (?, ?, ?, ?, ?, ?)',
  );
  seedProfessions.forEach((item) =>
    statement.run([
      item.id,
...
function migrateNightLordSkills(db: Database): boolean {
  const currentVersion = db.exec(
    "SELECT value FROM app_meta WHERE key = 'night_lord_seed_version'",
  )[0]?.values[0]?.[0];
  ...
  nightLordSeedSkills.forEach((item) =>
    statement.run([
      item.id,
      item.professionId,
      item.name,
...
function migrateAdeleSkills(db: Database): boolean {
  const currentVersion = db.exec("SELECT value FROM app_meta WHERE key = 'adele_seed_version'")[0]
    ?.values[0]?.[0];
  if (currentVersion === '1') return false;
...
function migrateBishopSkills(db: Database): boolean {
  const currentVersion = db.exec("SELECT value FROM app_meta WHERE key = 'bishop_seed_version'")[0]
    ?.values[0]?.[0];
  if (currentVersion === '1') return false;
...
function migrateIceLightningSkills(db: Database): boolean {
  const currentVersion = db.exec(
    "SELECT value FROM app_meta WHERE key = 'ice_lightning_seed_version'",
  )[0]?.values[0]?.[0];
  if (currentVersion === '1') return false;
  ...
  iceLightningSeedSkills.forEach((item) =>
    statement.run([
      item.id,
      item.professionId,
      item.name,
...
function migrateShadowerSkills(db: Database): boolean {
  const currentVersion = db.exec(
    "SELECT value FROM app_meta WHERE key = 'shadower_seed_version'",
  )[0]?.values[0]?.[0];
  if (currentVersion === '1') return false;
  ...
  shadowerSeedSkills.forEach((item) =>
    statement.run([
      item.id,
      item.professionId,
      item.name,
...
function migrateReferenceSkills(db: Database): boolean {
  const currentVersion = db.exec(
    "SELECT value FROM app_meta WHERE key = 'reference_skill_seed_version'",
  )[0]?.values[0]?.[0];
  ...
  seedSkills
    .filter((item) => professionIds.has(item.professionId))
    .forEach((item) =>
      statement.run([
        item.id,
        item.professionId,
  ...
  statement.free();
  db.run(
    "INSERT OR REPLACE INTO app_meta (key, value) VALUES ('reference_skill_seed_version', '2')",
  );
  return true;
}
...
export async function loadProfessions(): Promise<Profession[]> {
  return rows<Profession>(
    await database(),
    'SELECT id, name, category, icon_url AS iconUrl, sort_order AS sortOrder, active FROM professions WHERE active = 1 ORDER BY sort_order, name',
...
export async function loadAllProfessions(): Promise<Profession[]> {
  return rows<Profession>(
    await database(),
    'SELECT id, name, category, icon_url AS iconUrl, sort_order AS sortOrder, active FROM professions ORDER BY sort_order, name',
  ).map((item) => ({ ...item, active: Boolean(item.active) }));
}
...
export async function loadSkills(professionId: string, includeInactive = false): Promise<Skill[]> {
  ...
  const result = db.exec(
    `SELECT id, profession_id AS professionId, name, icon_url AS iconUrl, sort_order AS sortOrder, active, recommended FROM skills WHERE profession_id = '${professionId.replaceAll("'", "''")}'${activeClause} ORDER BY recommended DESC, sort_order, name`,
  )[0];
  if (!result) return [];
  return result.values.map((values) => ({
    ...Object.fromEntries(result.columns.map((column, index) => [column, values[index]])),
    active: Boolean(values[result.columns.indexOf('active')]),
    recommended: Boolean(values[result.columns.indexOf('recommended')]),
  })) as Skill[];
}
...
export async function saveSkill(item: Skill): Promise<void> {
  ...
  const db = await database();
  const statement = db.prepare(
    'INSERT OR REPLACE INTO skills (id, profession_id, name, icon_url, sort_order, active, recommended) VALUES (?, ?, ?, ?, ?, ?, ?)',
  );
  statement.run([
    item.id,
    item.professionId,
...
export async function deleteProfession(id: string): Promise<void> {
  assertPerfectCoreAdmin();
  const db = await database();
  const statement = db.prepare('DELETE FROM skills WHERE profession_id = ?');
  statement.run([id]);
  statement.free();
  const profession = db.prepare('DELETE FROM professions WHERE id = ?');
  profession.run([id]);
  profession.free();
  await writeSnapshot(db.export());
}
...
export async function exportDatabase(): Promise<Uint8Array> {
  assertPerfectCoreAdmin();
  return (await database()).export();
}

export async function resetDatabase(): Promise<void> {
  assertPerfectCoreAdmin();
  const db = await database();
  db.run('DELETE FROM skills; DELETE FROM professions; DELETE FROM app_meta;');
  seed(db);
  await writeSnapshot(db.export());
}
