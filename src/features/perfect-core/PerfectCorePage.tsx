...
import { calculatePerfectCores, getCoreKey, recommendNextCores } from './perfectCoreCalculator';
...
import type { BoostCore, CoreMode, Profession, Skill, PerfectCoreResult } from './perfectCoreTypes';
  ...
  return (
    <span className="core-skill">
      <span className="skill-avatar">
        {skill?.iconUrl ? <img src={assetPath(skill.iconUrl)} alt="" /> : skill?.name.slice(0, 1)}
      </span>
      <span>
...
const modeInfo: Record<CoreMode, { title: string; detail: string; count: number }> = {
...
export default function PerfectCorePage() {
  ...
  const [ownedFilter, setOwnedFilter] = useState<'all' | 'matched' | 'remaining'>('all');
  ...
  useEffect(() => {
    loadProfessions().then((items) => {
      ...
      setProfessionId(items.find((item) => item.id === 'night-lord')?.id ?? items[0]?.id ?? '');
    });
  }, []);
  useEffect(() => {
    if (professionId)
  ...
  const visibleProfessions = professions.filter(
    (profession) =>
      ...
      (profession.name.includes(search) || profession.category.includes(search)),
  );
  const scoredResults = useMemo<ScoredResult[]>(
    () =>
      results
        ...
        .sort((a, b) => a.replaceCount - b.replaceCount || b.matchedCount - a.matchedCount),
    [results, ownedCores],
  );
  ...
  const changeMode = (nextMode: CoreMode) => {
    ...
    const recommended = skills.filter((skill) => skill.recommended).map((skill) => skill.id);
    setMode(nextMode);
    setSelected((current) =>
    ...
    setMessage(
      nextMode === '4CORE_4' ? '4核4是过渡方案，需要后续转换为完美核心' : '选择目标技能后开始计算',
    );
  };
  const calculate = () => {
  ...
  const addOwnedCore = () => {
    const [main, sub1, sub2] = [ownedDraft.mainSkillId, ...ownedDraft.subSkillIds];
    if (!main || !sub1 || !sub2) return setMessage('请完整选择已有核心的 3 个技能');
    if (new Set([main, sub1, sub2]).size !== 3) return setMessage('一个核心内不能重复技能');
          ...
          <div className="category-filter">
            ...
            {availableCategories.map((category) => (
              ...
              >
                {category}
                <small>
                  {professions.filter((profession) => profession.category === category).length}
                </small>
              </button>
            ))}
          </div>
          ...
          <div className="profession-grid">
            {visibleProfessions.map((profession) => (
              <button
                key={profession.id}
                className={
                  profession.id === professionId ? 'profession-card active' : 'profession-card'
                }
            ...
            {visibleProfessions.length === 0 && <p className="profession-empty">没有匹配的职业</p>}
          </div>
        </aside>
        <section className="perfect-main">
          <div className="workspace-head">
            <div>
              ...
              <h2>{professions.find((item) => item.id === professionId)?.name ?? '选择职业'}</h2>
              {referenceLayout && (
          ...
          <div className="mode-groups">
            ...
            <section className="mode-group transition">
              ...
              <button
                className={
                  mode === '4CORE_4' ? 'mode transition-mode active' : 'mode transition-mode'
                }
          ...
          {mode === '4CORE_4' && (
            <div className="transition-note">
              ...
              <span>
                4 个技能各出现 3 次，用于前期过渡；有效强化按 50 级封顶，后续建议转为 3核4.5 或
                4核6。
              </span>
            </div>
          )}
          <div className="skill-toolbar">
            ...
            <input
              ...
              placeholder="搜索技能"
            />
            <button onClick={() => setSelected(visibleSkills.map((skill) => skill.id))}>
              全部选择
            </button>
              ...
              {visibleSkills.map((skill) => (
                ...
                >
                  ...
                  <span>
                    ...
                    <small>{selected.includes(skill.id) ? '已加入目标' : '点击选择'}</small>
                  </span>
          ...
          {mode === '3CORE_4_5' && (
            <div className="half-row">
              ...
              <select value={halfSkillId} onChange={(event) => setHalfSkillId(event.target.value)}>
          ...
          <div className="calculate-bar">
            <span className={message.startsWith('找到') ? 'success' : ''}>{message}</span>
            <button className="primary-action" onClick={calculate}>
              {mode === '4CORE_4' ? '生成过渡核心' : '生成完美核心'} <span>→</span>
            </button>
          </div>
          {results.length > 0 && (
            <>
              <OwnedCorePanel
                ...
                onAdd={addOwnedCore}
                onRemove={(index) =>
                  setOwnedCores((current) => current.filter((_, itemIndex) => itemIndex !== index))
                }
              />
              <NextCorePanel
    ...
    <section className="owned-section">
      ...
      {ownedCores.map((core, index) => (
        <div className="owned-core-row" key={`${getCoreKey(core)}-${index}`}>
          ...
          <SkillBadge skill={skills.find((skill) => skill.id === core.mainSkillId)} label="主" />
          <SkillBadge skill={skills.find((skill) => skill.id === core.subSkillIds[0])} />
          <SkillBadge skill={skills.find((skill) => skill.id === core.subSkillIds[1])} />
      ...
      <div className="owned-core-editor">
        ...
        <select
          ...
          onChange={(event) => setDraft({ ...draft, mainSkillId: event.target.value })}
        >
          {optionList}
        </select>
        <select
  ...
  return (
    <section className="next-core-section">
      <div className="results-heading">
        ...
        <span>{recommendations.length ? '根据当前已拥有核心反推' : '暂无匹配方案'}</span>
      </div>
        ...
        <div className="next-core-list">
          {recommendations.slice(0, 8).map((item, index) => (
            ...
            >
              ...
              <div>
                <SkillBadge
                  skill={skills.find((skill) => skill.id === item.core.mainSkillId)}
                  label="主"
                />
                <SkillBadge skill={skills.find((skill) => skill.id === item.core.subSkillIds[0])} />
                <SkillBadge skill={skills.find((skill) => skill.id === item.core.subSkillIds[1])} />
              </div>
              <span>
...
function ResultList({
  ...
  return (
    <section className="results-section">
      <div className="results-heading">
        <div>
          <span className="eyebrow">
            {mode === '4CORE_4' ? '04 / TRANSITION LAYOUTS' : '04 / CALCULATED LAYOUTS'}
          </span>
          <h2>
      ...
      {ownedCount > 0 && (
        <p className="result-note">已按最少替换核心数排序；每个方案标记可直接保留的核心。</p>
      )}
        ...
        {results.slice(0, visible).map((result, index) => (
          ...
          >
            ...
            <div className="result-meta">
              {ownedCount > 0 && (
                <strong>
                  可保留 {result.matchedCount} 核 · 需替换 {result.replaceCount} 核
                </strong>
              )}
