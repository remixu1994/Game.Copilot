    ...
    items: [
      {
        name: '属性攻击力',
        description:
          '属性攻击力越高，技能伤害越强。根据基本属性和攻击力数值，获得各职业对应的属性（物理 / 魔法）。',
        tag: '伤害',
      },
      {
        name: '主属性',
        description: '通过角色升级、装备、各种成长获得。主属性可以大幅提高属性攻击力。',
        tag: '成长',
      },
      {
        name: '副属性',
        description: '副属性额外提高属性攻击力，与职业的主属性共同参与角色面板计算。',
        tag: '成长',
      },
    ],
  },
  {
    ...
    items: [
      ...
      { name: '最终伤害', description: '经过技能、被动与各项增益结算后的最终伤害修正。' },
      {
        name: '无视防御率',
        description:
          '无视敌人防御率的比例。对怪物、玩家和技能发动的无视防御率进行乘算后适用，最高适用 100%。',
      },
    ],
  },
  {
  ...
  return (
    <main className="data-page">
      <header className="data-topbar">
        <a className="data-brand" href={sitePath('/')}>
          Maple<span>Lab</span>
        </a>
        <nav>
          <a href={sitePath('/dps')}>伤害 DPS 模拟器</a>
          <a href={sitePath('/currency')}>枫币计算器</a>
          <a className="current" href={sitePath('/data')}>
            基础数据
          </a>
        </nav>
        <button type="button" aria-label="打开搜索">
          ⌕
        </button>
      </header>
      <div className="data-layout">
        ...
        <article className="data-article">
          ...
          {filtered.length === 0 && (
            <div className="empty-data">没有找到匹配的属性，请尝试其他关键词。</div>
          )}
