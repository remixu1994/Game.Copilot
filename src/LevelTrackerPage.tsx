...
const defaultBaseline = () => {
  const startDate = baselineDateForToday(localTodayKey(), defaultSettings.startDate, defaultSettings.endDate);
  return {
    startDate,
    baselineTiming: startDate === defaultSettings.startDate ? defaultSettings.baselineTiming : 'start-of-day' as const,
  };
};
const cloneDefaults = (): StoredTracker => {
  const settings = { ...defaultSettings, ...defaultBaseline(), levelRequirements: { ...defaultSettings.levelRequirements } };
...
const hydrateLevelRequirements = (requirements?: Record<string, number>) => {
  ...
  const shouldMigrateLevel203 = !stored['203'] || stored['203'] === 420_000_000_000 || stored['203'] === previousLevel203;
  const level203 = shouldMigrateLevel203
    ? defaultSettings.levelRequirements['203']
    : stored['203'];
  const previousSuggested206 = extrapolateLevel206Requirement(level200, previousLevel203);
  const level206 = !stored['206'] || (shouldMigrateLevel203 && stored['206'] === previousSuggested206)
...
const hydrateRecords = (settings: TrackerSettings, sources: ExperienceSource[], records: DailyRecord[], migrateWeeklySettlementDates = false) => {
  const weeklyIds = new Set(sources.filter((source) => source.frequency === 'weekly').map((source) => source.id));
  const baselineEligibleWeeklyIds = new Set(sources.filter((source) => source.frequency === 'weekly' && (
    source.group === 'weekly-dungeon' || (source.rewardTier ?? source.optionalPurchase?.rewardTier) === 1
  )).map((source) => source.id));
  const monsterPark = sources.find((source) => source.id === 'monster-park' && source.calculation?.kind === 'per-ticket');
  const monsterParkPerRunExp = monsterPark?.calculation?.kind === 'per-ticket'
  ...
  const baselineWeeklyIds = new Set((baselineRecord?.baselineIncludedSourceIds ?? [])
    .filter((id) => weeklyIds.has(id) && baselineEligibleWeeklyIds.has(id)));
  const selectedFirstWeekIds = new Set(records
  ...
  return records.map((record) => {
    ...
    if (sourceExpOverrides?.['monster-park'] !== undefined && sourceUnitOverrides?.['monster-park'] === undefined && monsterParkPerRunExp > 0) {
      sourceUnitOverrides = { ...sourceUnitOverrides, 'monster-park': Math.max(0, Math.round(sourceExpOverrides['monster-park'] / monsterParkPerRunExp)) };
      sourceExpOverrides = { ...sourceExpOverrides };
      delete sourceExpOverrides['monster-park'];
    }
    if (migrateWeeklySettlementDates && settings.baselineTiming === 'end-of-day' && weekKey(record.date) === firstWeek) {
      ...
      if (record.date === settings.startDate) {
        ...
        if (baselineIncludedSourceIds) {
          baselineIncludedSourceIds = baselineIncludedSourceIds.filter((id) => !weeklyIds.has(id) || baselineEligibleWeeklyIds.has(id));
        } else {
        ...
        completedSourceIds = [...dailyIds, ...weeklyIdsForDate.filter((id) => !baselineWeeklyIds.has(id))];
      }
    } else if (record.date === settings.startDate) {
      baselineIncludedSourceIds = baselineIncludedSourceIds
        ? baselineIncludedSourceIds.filter((id) => !weeklyIds.has(id) || baselineEligibleWeeklyIds.has(id))
        : settings.baselineTiming === 'end-of-day' ? [...completedSourceIds] : [];
    }
    return { ...record, completedSourceIds, baselineIncludedSourceIds, sourceExpOverrides, sourceUnitOverrides };
  });
};
const migratePreviousBaseline = (settings: TrackerSettings, records: DailyRecord[]) => records.map((record) => record.date === settings.startDate
  ? { ...record, baselineIncludedSourceIds: settings.baselineTiming === 'end-of-day' ? [...record.completedSourceIds] : [] }
  : record);
...
const rebaseRecords = (
  ...
  const previousWeeklyIds = new Set(records
  ...
  const rebased = defaults.map((record) => {
    ...
    if (record.date === startDate) {
      if (!hasPreviousWeek) return existing ? { ...record, sourceExpOverrides: existing.sourceExpOverrides, sourceUnitOverrides: existing.sourceUnitOverrides } : record;
      const dailyIds = record.completedSourceIds.filter((id) => sources.some((source) => source.id === id && source.frequency === 'daily'));
      const completedSourceIds = [...dailyIds, ...previousWeeklyIds];
      const baselineIncludedSourceIds = baselineTiming === 'end-of-day'
        ? [...completedSourceIds]
        : [];
      return { ...record, completedSourceIds, baselineIncludedSourceIds, sourceExpOverrides: existing?.sourceExpOverrides, sourceUnitOverrides: existing?.sourceUnitOverrides };
    }
    return existing ? { ...record, completedSourceIds: existing.completedSourceIds, actualPercent: existing.actualPercent, sourceExpOverrides: existing.sourceExpOverrides, sourceUnitOverrides: existing.sourceUnitOverrides } : record;
  });
  ...
  try {
    ...
    if (previousSaved) {
      ...
      if (parsed.settings && Array.isArray(parsed.sources) && Array.isArray(parsed.records)) {
        const sources = hydrateSources(parsed.sources);
        const settings = hydrateSettings(parsed.settings);
        return { ...parsed, settings, sources, records: hydrateRecords(settings, sources, parsed.records, true) };
      }
    }
    ...
    if (baselineSaved) {
      ...
      if (parsed.settings && Array.isArray(parsed.sources) && Array.isArray(parsed.records)) {
        const sources = hydrateSources(parsed.sources);
        const settings = hydrateSettings(parsed.settings);
        return { ...parsed, settings, sources, records: hydrateRecords(settings, sources, migratePreviousBaseline(settings, parsed.records), true) };
      }
    }
...
function TaskCheck({ source, checked, disabled = false, statusText, onToggle }: { source: ExperienceSource; checked: boolean; disabled?: boolean; statusText?: string; onToggle: () => void }) {
  return <label className={`${source.frequency === 'weekly' ? 'weekly-check' : ''} ${disabled ? 'baseline-locked' : ''}`}>
    <input type="checkbox" checked={checked} disabled={disabled} onChange={onToggle} />
    <span><strong>{sourceDisplayName(source)}</strong><small>{formatExp(sourceExperience(source))} · {statusText ?? (source.frequency === 'weekly' ? '每周' : '每日')}</small></span>
  </label>;
}

function DailyTaskList({ sources, completedIds, expOverrides, unitOverrides, allowExpOverride, onToggle, onExpOverride, onUnitOverride }: { sources: ExperienceSource[]; completedIds: string[]; expOverrides?: Record<string, number>; unitOverrides?: Record<string, number>; allowExpOverride: boolean; onToggle: (source: ExperienceSource) => void; onExpOverride: (source: ExperienceSource, value: string) => void; onUnitOverride: (source: ExperienceSource, value: string) => void }) {
  ...
  return <section className="task-group daily-task-group">
    ...
    <div className="task-check-grid">{daily.map((source) => {
      if (source.id === 'monster-park' && source.calculation?.kind === 'per-ticket') {
        ...
        return <div className={`daily-exp-override ${unitOverrides?.[source.id] !== undefined ? 'customized' : ''}`} key={source.id}>
          <TaskCheck source={dailySource} checked={isChecked(source)} onToggle={() => onToggle(source)} />
          <span className="today-exp-input monster-run-input"><small>{allowExpOverride ? `今日 ${formatExp(calculatedExp)}` : '已计入基准'}</small><span><input aria-label="今日怪物乐园次数" type="number" min="0" step="1" disabled={!isChecked(source) || !allowExpOverride} value={units} onChange={(event) => onUnitOverride(source, event.target.value)} /><em>次</em></span></span>
        </div>;
      }
      return source.id === 'auto-6h'
        ? <div className={`daily-exp-override ${expOverrides?.[source.id] !== undefined ? 'customized' : ''}`} key={source.id}>
          <TaskCheck source={source} checked={isChecked(source)} onToggle={() => onToggle(source)} />
          <span className="today-exp-input"><small>{allowExpOverride ? '今日挂机经验' : '已计入基准'}</small><span><input aria-label="今日挂机经验" type="number" min="0" step="0.01" disabled={!isChecked(source) || !allowExpOverride} value={(expOverrides?.[source.id] ?? sourceExperience(source)) / EXP_PER_YI} onChange={(event) => onExpOverride(source, event.target.value)} /><em>亿</em></span></span>
        </div>
        : <TaskCheck key={source.id} source={source} checked={isChecked(source)} onToggle={() => onToggle(source)} />;
    })}</div>
  </section>;
}

function WeekTaskPanel({ sources, completedIds, baselineIncludedIds, completionDates, weekDates, showDungeonBaselineControl, onToggle, onCompletionDateChange, onDungeonBaselineChange }: { sources: ExperienceSource[]; completedIds: string[]; baselineIncludedIds: string[]; completionDates: Record<string, string>; weekDates: string[]; showDungeonBaselineControl: boolean; onToggle: (source: ExperienceSource) => void; onCompletionDateChange: (source: ExperienceSource, date: string) => void; onDungeonBaselineChange: (source: ExperienceSource, included: boolean) => void }) {
  ...
  const baselineExp = sources.filter((source) => isChecked(source) && baselineIncluded.has(source.id)).reduce((sum, source) => sum + sourceExperience(source), 0);
  const newWeeklyExp = sources.filter((source) => isChecked(source) && !baselineIncluded.has(source.id)).reduce((sum, source) => sum + sourceExperience(source), 0);
  const claimDateSelect = (source: ExperienceSource) => {
    if (!isChecked(source)) return null;
    if (baselineIncluded.has(source.id)) return <span className="weekly-claim-baseline">基准日</span>;
    return <label className="weekly-claim-date"><span>领取日</span><select aria-label={`${source.name}领取日期`} value={completionDates[source.id] ?? weekDates.at(-1)} onChange={(event) => onCompletionDateChange(source, event.target.value)}>{weekDates.map((date) => <option value={date} key={date}>{formatShortDate(date)}</option>)}</select></label>;
  };

  return <div className={`week-task-panel ${baselineExp > 0 ? 'baseline-included' : ''}`}>
    <div className="week-task-panel-head"><span><b>WEEKLY</b><strong>本周周常汇总</strong></span><small>基础 {completedCount} / {weeklyDungeons.length + weeklyRewards.length} · 基准已含 {formatExp(baselineExp)} · 本周新增 {formatExp(newWeeklyExp)}</small></div>
    <div className="week-task-panel-grid">
      <section className="task-group weekly-dungeon-group">
        ...
        <div className="task-check-grid weekly-dungeon-grid">{weeklyDungeons.map((source) => {
          const checked = isChecked(source);
          const included = baselineIncluded.has(source.id);
          return <div className={`weekly-dungeon-item ${included ? 'baseline-item' : ''} ${showDungeonBaselineControl ? 'has-baseline-control' : ''}`} key={source.id}>
            <TaskCheck source={source} checked={checked} statusText={included ? '已计入当前基准' : '本周新增'} onToggle={() => onToggle(source)} />
            {claimDateSelect(source)}
            {showDungeonBaselineControl && <label className="baseline-toggle item-baseline-toggle"><input type="checkbox" checked={checked && included} disabled={!checked} onChange={(event) => onDungeonBaselineChange(source, event.target.checked)} /><span>纳入基准</span></label>}
          </div>;
        })}</div>
      </section>
      ...
      <div className="weekly-reward-table">{weeklyRewards.map((base) => {
        ...
        return <div className="weekly-reward-row" key={base.id}>
          ...
          <div className="reward-choice-cell base-reward"><label className="reward-choice"><input type="checkbox" checked={isChecked(base)} onChange={() => onToggle(base)} /><span><strong>基础领取</strong><small>{baselineIncluded.has(base.id) ? '已计入当前基准' : '领取后计入本周'}</small></span></label>{claimDateSelect(base)}</div>
          {purchase && <div className="reward-choice-cell purchase-reward"><label className="reward-choice"><input type="checkbox" checked={isChecked(purchase)} onChange={() => onToggle(purchase)} /><span><strong>额外购买一次</strong><small>{baselineIncluded.has(purchase.id) ? '已计入当前基准' : `${purchaseLabel(purchase)} · +${formatExp(sourceExperience(purchase))}`}</small></span></label>{claimDateSelect(purchase)}</div>}
        </div>;
      })}</div>
      </section>
    </div>
...
export default function LevelTrackerPage() {
  ...
  const [openWeek, setOpenWeek] = useState<string | null>(() => weekKey(tracker.settings.startDate));
  ...
  const updateBaseline = (startDate: string, baselineTiming: TrackerSettings['baselineTiming']) => {
    ...
    setTracker((current) => ({
      ...
      records: rebaseRecords(current.settings, current.sources, current.records, startDate, baselineTiming),
    }));
  ...
  const toggleTask = (date: string, source: ExperienceSource) => {
    setTracker((current) => {
      ...
      const nextRecords = current.records.map((record) => {
        ...
        if (record.date === scheduleDate && !selected) completedSourceIds = [...completedSourceIds, source.id];
  ...
  const updateWeeklyDungeonBaseline = (targetWeek: string, sourceId: string, included: boolean) => {
    setTracker((current) => {
      ...
      if (!source || source.frequency !== 'weekly' || source.group !== 'weekly-dungeon') return current;
      ...
      const currentCompletionDate = weekRecords.find((record) => record.completedSourceIds.includes(sourceId))?.date;
      const latestCalibrationDate = [...weekRecords].reverse().find((record) => record.date !== current.settings.startDate && record.actualPercent !== null)?.date;
      ...
      return {
        ...
        records: current.records.map((record) => {
          if (weekKey(record.date) !== targetWeek) return record;
          const completedSourceIds = record.completedSourceIds.filter((id) => id !== sourceId);
          const baselineIncludedSourceIds = record.date === current.settings.startDate
            ? (record.baselineIncludedSourceIds ?? record.completedSourceIds).filter((id) => id !== sourceId)
            : record.baselineIncludedSourceIds;
          return {
            ...record,
            completedSourceIds: selected && record.date === nextCompletionDate ? [...completedSourceIds, sourceId] : completedSourceIds,
            baselineIncludedSourceIds: record.date === current.settings.startDate && included && selected
  ...
  const resetTracker = () => {
    ...
    setSourcesDirty(false);
    [STORAGE_KEY, PREVIOUS_STORAGE_KEY, BASELINE_STORAGE_KEY, LEGACY_STORAGE_KEY, OLDER_STORAGE_KEY].forEach((key) => window.localStorage.removeItem(key));
  ...
  return <main className="tracker-shell">
    <header className="tracker-header">
      ...
      <div className="tracker-heading"><span className="tracker-eyebrow">PROGRESSION / DAILY FORECAST</span><h1>等级跟踪器</h1><p>从当前经验基准出发，自动忽略更早日期并预测到 {formatDate(settings.endDate)}。</p></div>
      <a className="tracker-back" href={sitePath('/')}>返回工具目录 ↗</a>
    </header>

    <section className="tracker-command">
      <div className="command-copy"><span className="tracker-eyebrow">CURRENT BASELINE / {formatShortDate(settings.startDate)} {settings.baselineTiming === 'end-of-day' ? 'END OF DAY' : 'START OF DAY'}</span><h2>{formatProgress({ level: settings.currentLevel, percent: settings.currentPercent })}</h2><p>{formatDate(settings.startDate)}之前的日期不再参与预测；{settings.baselineTiming === 'end-of-day' ? '当日所有已勾选任务均已包含在当前经验中。' : '当日任务将从当前经验继续累计。'}</p></div>
      <div className="settings-line">
        <label><span>当前数据日期</span><input type="date" min={defaultSettings.startDate} max={settings.endDate} value={settings.startDate} onChange={(event) => updateBaseline(event.target.value, settings.baselineTiming)} /></label>
        <label><span>当日经验状态</span><select value={settings.baselineTiming} onChange={(event) => updateBaseline(settings.startDate, event.target.value as TrackerSettings['baselineTiming'])}><option value="start-of-day">日初 · 今日尚未计算</option><option value="end-of-day">日终 · 今日已包含</option></select></label>
    ...
    <section className={`final-forecast ${targetReached ? 'reached' : 'short'}`}>
      <div><span className="tracker-eyebrow">FINAL / {settings.endDate.replaceAll('-', '.')}</span><h2>{formatProgress(projection.final)}</h2><p>{targetReached ? `预计 ${formatDate(projection.reachedTargetDate ?? settings.endDate)} 达到 Lv.${settings.targetLevel}` : `按当前计划尚未达到 Lv.${settings.targetLevel}`}</p></div>
    ...
    <section className="source-editor">
      <button className="source-editor-toggle" type="button" onClick={() => setSourcesOpen(!sourcesOpen)}><span><span className="tracker-eyebrow">01 / EXPERIENCE SOURCES</span><strong>任务经验设置</strong><small>完成修改后，点击刷新同步至每日与每周计划</small></span><b>{sourcesOpen ? '收起 ↑' : '展开编辑 ↓'}</b></button>
    ...
    <section className="weekly-workspace">
      <div className="section-intro"><div><span className="tracker-eyebrow">02 / WEEKLY TRACKING</span><h2>按周执行与校准</h2></div><p>为周常设置实际领取日；若当天已填写日终校准，周常经验视为已包含，不会再次累计。</p></div>
      <div className="week-list">{projection.weeks.map((week, weekIndex) => {
        ...
        const weekBaselineIncludedIds = [...new Set(records
          .filter((record) => weekKey(record.date) === week.key)
          .flatMap((record) => record.baselineIncludedSourceIds ?? []))];
        const showDungeonBaselineControl = week.key === weekKey(settings.startDate) && settings.baselineTiming === 'end-of-day';
        const completionDates = Object.fromEntries(records
          .filter((record) => weekKey(record.date) === week.key)
          .flatMap((record) => record.completedSourceIds
            .filter((id) => sources.some((source) => source.id === id && source.frequency === 'weekly'))
            .map((id) => [id, record.date])));
        const claimableWeekDates = week.days.map((day) => day.date).filter((date) => !(showDungeonBaselineControl && date === settings.startDate));
        const latestCalibratedDate = [...week.days].reverse().find((day) => day.calibrated)?.date;
        const weeklyScheduleDate = latestCalibratedDate ?? claimableWeekDates.at(-1) ?? week.endDate;
        return <article className={`week-section ${expanded ? 'expanded' : ''}`} key={week.key}>
          <button className="week-summary" type="button" onClick={() => setOpenWeek(expanded ? null : week.key)}>
            <span className="week-number">0{weekIndex + 1}</span><span className="week-period"><small>{formatShortDate(week.startDate)} — {formatShortDate(week.endDate)}</small><strong>第 {weekIndex + 1} 周</strong></span><span><small>期初</small><strong>{formatProgress(week.start)}</strong></span><span><small>日常 / 周常新增</small><strong>{formatExp(week.dailyEarnedExp)} / {formatExp(week.weeklyEarnedExp)}</strong></span><span><small>本周结束预测</small><strong className="forecast-value">{formatProgress(week.end)}</strong></span><b>{expanded ? '−' : '+'}</b>
          </button>
          {expanded && <div className="week-days"><WeekTaskPanel sources={sources.filter((source) => source.enabled && source.frequency === 'weekly')} completedIds={weekCompletedIds} baselineIncludedIds={weekBaselineIncludedIds} completionDates={completionDates} weekDates={claimableWeekDates} showDungeonBaselineControl={showDungeonBaselineControl} onToggle={(source) => toggleTask(weeklyScheduleDate, source)} onCompletionDateChange={(source, date) => moveWeeklyTaskToDate(week.key, source.id, date)} onDungeonBaselineChange={(source, included) => updateWeeklyDungeonBaseline(week.key, source.id, included)} /><div className="day-row day-head"><span>日期</span><span>日常任务 / 周常状态</span><span>日常经验</span><span>日常预测</span><span>实际日终经验</span><span /></div>{week.days.map((day) => {
            ...
            const weeklyStatus = day.claimedWeeklySourceIds.length > 0
              ? day.calibrated ? `今日领取 ${day.claimedWeeklySourceIds.length} 项 · 已含校准` : `今日领取 ${day.claimedWeeklySourceIds.length} 项`
              : day.completedWeeklySourceIds.length > 0 ? `周常已完成 ${day.completedWeeklySourceIds.length} 项` : '周常待完成';
            return <div className={`day-block ${day.calibrated ? 'calibrated' : ''}`} key={day.date}><div className="day-row"><button className="date-button" type="button" onClick={() => setOpenDay(dayExpanded ? null : day.date)}><small>{new Intl.DateTimeFormat('zh-CN', { weekday: 'short' }).format(new Date(`${day.date}T00:00:00`))}</small><strong>{formatShortDate(day.date)}</strong></button><span className="task-count"><span><b>{activeCompleted.length}</b> 项日常</span><small className={day.claimedWeeklySourceIds.length > 0 ? 'claimed' : ''}>{weeklyStatus}</small></span><span className="day-exp"><strong>{day.date === settings.startDate ? (day.earnedExp > 0 ? `今日 ${formatExp(day.earnedExp)}` : '基准日') : formatExp(day.earnedExp)}</strong>{day.weeklyEarnedExp > 0 && <small>{day.calibrated ? '周常经验已由校准吸收' : `周常另计 +${formatExp(day.weeklyEarnedExp)}`}</small>}</span><span className="day-result"><small>{formatProgress(day.start)}</small><b>→ {formatProgress(day.predictedEnd)}</b></span>{day.date === settings.startDate ? <span className="baseline-label">{settings.baselineTiming === 'end-of-day' ? '日终基准' : '从今日开始'}</span> : <label className="actual-input"><input aria-label={`${formatDate(day.date)}实际日终经验百分比`} type="number" min="0" max="100" step="0.01" placeholder="填写校准" value={record.actualPercent ?? ''} onChange={(event) => updateActualPercent(day.date, event.target.value)} /><em>%</em></label>}<button className="expand-day" type="button" onClick={() => setOpenDay(dayExpanded ? null : day.date)}>{dayExpanded ? '收起' : day.calibrated ? '已校准' : '日常'}</button></div>{dayExpanded && <div className="day-tasks"><div className="day-task-note"><strong>{formatDate(day.date)}</strong><span>{day.date === settings.startDate ? (settings.baselineTiming === 'end-of-day' ? '当日全部已勾选任务均已包含在当前基准，不会重复累计。' : '当日任务将从当前经验继续累计。') : day.calibrated ? `实际日终经验已校准为 ${record.actualPercent?.toFixed(2)}%，当天领取的周常经验也已包含其中。` : '取消勾选会立即从当日及后续预测中扣除；填写实际百分比可校准后续预测。'}</span></div><DailyTaskList sources={sources.filter((source) => source.enabled)} completedIds={record.completedSourceIds} expOverrides={record.sourceExpOverrides} unitOverrides={record.sourceUnitOverrides} allowExpOverride={day.date !== settings.startDate || settings.baselineTiming === 'start-of-day'} onToggle={(source) => toggleTask(day.date, source)} onExpOverride={(source, value) => updateDailySourceExperience(day.date, source, value)} onUnitOverride={(source, value) => updateDailySourceUnits(day.date, source, value)} /></div>}</div>;
          })}</div>}
        </article>;
      })}</div>
    </section>

    <footer className="tracker-footer"><span>BASELINE / {settings.startDate.replaceAll('-', '.')}</span><button type="button" onClick={resetTracker}>恢复默认数据</button><span>经验单位：亿 / 万亿 · 数据保存在当前浏览器</span></footer>
  </main>;
}
