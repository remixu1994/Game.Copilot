...
type MaterialTone = 'powder' | 'purple' | 'gold';
...
const powderSources: PowderSource[] = [
  ...
  { id: 'weapon-artifact-powder', shop: '水晶商店', name: '神秘的武器炼成粉（神器）', image: '/items/artifact-weapon-powder-detail.png', quantity: 7, experienceEach: 1000, redDiamondCost: 14, equipment: 'weapon' },
...
export default function EquipmentUpgradePage() {
  ...
  const metrics = useMemo(() => {
    ...
    const createMaterial = (name: string, shortName: string, percent: number, cost: number | null, costUnit: string, tone: MaterialTone, explicitMesoCost?: number): MaterialResult => {
      ...
      const experience = experiencePerPercent * percent;
  ...
  }, [armorStonePrice, artifactCost, artifactPercent, economy.rmbPerMeso, economy.rmbPerRedCrystal, powderExperience, powderPercent, purplePercent, steelCount, steelPackMesoPrice, targetPercent, weaponArtifactPowderPrice, weaponPowderPrice, weaponStonePrice]);
  ...
  return <main className="upgrade-shell">
    ...
    <section className="upgrade-hero unified-hero">
    ...
    <section className="upgrade-materials">
      <div className="upgrade-section-heading"><div><p className="upgrade-eyebrow">UPGRADE MATERIALS</p><h2>装备升级材料</h2></div><span>统一按人民币成本比较经验效率</span></div>
      <div className="unified-material-grid">
          {metrics.materials.map((item) => <article className={`weapon-material-card ${item.tone}`} key={item.name}>
            <div className="material-title"><span>{item.shortName}</span><div><small>升级材料</small><h3>{item.name}</h3></div></div>
            {item.name === '史诗武器'
              ? <img className="material-reference-image epic" src="/items/epic-weapon-upgrade.png" alt="史诗武器升级材料截图" />
              : <img className="material-reference-image artifact" src="/items/artifact-weapon.png" alt="神器武器伽耶汉斗拳" />}
            <strong className="material-experience">{format(item.experience)}<small>经验</small></strong>
            <dl><div><dt>材料成本</dt><dd>{item.cost === null ? '—' : `${format(item.cost)} ${item.costUnit}`}</dd></div>{item.name === '史诗武器' && <div className="material-meso-editor steel-price-editor"><dt>钢铁售价</dt><dd><select aria-label="每10个钢铁售价" value={steelPackMesoPrice} onChange={(event) => setSteelPackMesoPrice(Number(event.target.value))}>{[5, 6, 7, 8].map((price) => <option value={price} key={price}>{price}</option>)}</select><span>枫币 / 10个</span></dd></div>}<div className={item.name === '神器武器' ? 'material-meso-editor' : ''}><dt>枫币成本</dt><dd>{item.name === '神器武器' ? <><select aria-label="神器武器枫币成本" value={artifactCost} onChange={(event) => setArtifactCost(Number(event.target.value))}>{[136, 203, 271, 339, 475].map((price) => <option value={price} key={price}>{price}</option>)}</select><span>枫币</span></> : `${format(item.mesoCost ?? 0, 2)} 枫币`}</dd></div><div><dt>人民币成本</dt><dd>{item.rmbCost === null ? '—' : `¥ ${format(item.rmbCost, 3)}`}</dd></div><div><dt>每元经验</dt><dd>{format(item.experiencePerRmb)}</dd></div></dl>
          </article>)}
          {armorMaterials.map((item, index) => <article className={`weapon-material-card armor-material-card ${item.tone}`} key={item.name}>
            <div className="material-title"><span>{item.shortName}</span><div><small>升级材料</small><h3>{item.name}</h3></div></div>
            <strong className="material-experience">{format(item.experience)}<small>经验</small></strong>
            <dl>
              ...
              <div><dt>每元经验</dt><dd>{item.experiencePerRmb === null ? '待补充' : format(item.experiencePerRmb)}</dd></div>
            </dl>
          </article>)}
      </div>
    </section>

    <section className="powder-sources">
      <div className="upgrade-section-heading"><div><p className="upgrade-eyebrow">POWDER MATERIALS</p><h2>粉尘升级材料</h2></div><span>红水晶价格支持编辑并实时换算</span></div>
      <div className="powder-source-grid">
        {upgradePowderSources.map((source) => <article className="powder-source-card" key={source.id}>
          <div className="source-product">
            <img className={source.id === 'weapon-artifact-powder' || source.id === 'armor-stone' ? 'detail-item-image' : ''} src={source.image} alt={`${source.name}商品图`} />
            <div><div className="source-heading"><span>{source.shop}</span><small>升级材料</small></div><h3>{source.name} ×{source.quantity}</h3></div>
          </div>
          <div className="source-total"><small>整组总经验</small><strong>{format(source.totalExperience)}</strong><em>经验</em></div>
          <dl>
            ...
            <div><dt>每元经验</dt><dd>{format(source.experiencePerRmb)}</dd></div>
          </dl>
        </article>)}
      </div>
    </section>
