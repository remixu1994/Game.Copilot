...
import { addExperience, baselineDateForToday, createDefaultRecords, defaultSettings, defaultSources, EXP_PER_YI, extrapolateLevel206Requirement, projectTracker, requiredExperienceForLevel, resolveWeeklyScheduleDate, sourceExperience, sourceTotals, weekKey, type ExperienceSource } from './levelTrackerCalculator';
...
describe('level tracker calculator', () => {
  ...
  it('counts baseline-day tasks only when the baseline is at the start of day', () => {
    ...
    expect(startProjection.final.level > endProjection.final.level
      || (startProjection.final.level === endProjection.final.level && startProjection.final.percent > endProjection.final.percent)).toBe(true);
  });
  ...
  it('recalculates after a source is edited or disabled', () => {
    ...
    expect(
      ...
      || (recalculated.final.level === baseline.final.level && recalculated.final.percent < baseline.final.percent),
    ).toBe(true);
  });
  ...
  it('uses a per-day idle experience override without changing other days', () => {
    ...
    const records = createDefaultRecords(defaultSettings, defaultSources);
    const baseline = projectTracker(defaultSettings, defaultSources, records);
    const customizedRecords = records.map((record) => record.date === '2026-08-26'
    ...
    expect(august26.earnedExp).toBeCloseTo(baseline.days.find((day) => day.date === '2026-08-26')!.earnedExp - sourceExperience(idle) + 100 * EXP_PER_YI, 2);
    expect(august27.earnedExp).toBe(baseline.days.find((day) => day.date === '2026-08-27')!.earnedExp);
  ...
  it('calculates a per-day monster park total from the overridden run count', () => {
    const monsterPark = defaultSources.find((source) => source.id === 'monster-park')!;
    const threeRuns = sourceExperience({ ...monsterPark, calculation: { ...monsterPark.calculation!, units: 3 } as ExperienceSource['calculation'] });
    const records = createDefaultRecords(defaultSettings, defaultSources);
    const baseline = projectTracker(defaultSettings, defaultSources, records);
    const customizedRecords = records.map((record) => record.date === '2026-08-26'
    ...
    expect(customized.days.find((day) => day.date === '2026-08-26')!.earnedExp).toBeCloseTo(
      baseline.days.find((day) => day.date === '2026-08-26')!.earnedExp - sourceExperience(monsterPark) + threeRuns,
      2,
    );
  ...
  it('checks every optional weekly purchase once per week by default', () => {
    ...
    expect(records.filter((record) => record.completedSourceIds.includes('weekly-reward-2-extra'))).toHaveLength(2);
    expect(records.filter((record) => record.completedSourceIds.includes('weekly-reward-3-extra'))).toHaveLength(2);
  ...
  it('treats first-week reward two as newly earned rather than baseline experience', () => {
    ...
    const claimRecord = records.find((record) => record.completedSourceIds.includes('weekly-reward-2'))!;
    const projection = projectTracker(defaultSettings, defaultSources, records);
    ...
    expect(projection.days.find((day) => day.date === '2026-08-26')!.earnedExp / EXP_PER_YI).toBeCloseTo(sourceTotals(defaultSources).daily / EXP_PER_YI, 6);
  });

  it('can exclude completed weekly dungeons from the baseline without changing daily experience', () => {
    const weeklyDungeonIds = new Set(defaultSources.filter((source) => source.group === 'weekly-dungeon').map((source) => source.id));
    const weeklyDungeonExp = defaultSources.filter((source) => weeklyDungeonIds.has(source.id)).reduce((sum, source) => sum + sourceExperience(source), 0);
    const records = createDefaultRecords(defaultSettings, defaultSources);
    const excludedRecords = records.map((record) => record.date === defaultSettings.startDate
      ? { ...record, baselineIncludedSourceIds: record.baselineIncludedSourceIds?.filter((id) => !weeklyDungeonIds.has(id)) }
      : record);
    ...
    expect(excluded.days.find((day) => day.date === '2026-08-26')!.earnedExp).toBe(included.days.find((day) => day.date === '2026-08-26')!.earnedExp);
    expect(excluded.weeks[0].weeklyEarnedExp - included.weeks[0].weeklyEarnedExp).toBeCloseTo(weeklyDungeonExp, 2);
  ...
  it('can configure each weekly dungeon baseline status independently', () => {
    ...
    const records = createDefaultRecords(defaultSettings, defaultSources);
    const updatedRecords = records.map((record) => record.date === defaultSettings.startDate
      ? { ...record, baselineIncludedSourceIds: record.baselineIncludedSourceIds?.filter((id) => id !== excludedDungeon.id) }
      : record);
    const baseline = projectTracker(defaultSettings, defaultSources, records);
    const updated = projectTracker(defaultSettings, defaultSources, updatedRecords);
    const baselineRecord = updatedRecords.find((record) => record.date === defaultSettings.startDate)!;
    ...
    expect(updated.days.find((day) => day.date === '2026-08-26')!.earnedExp).toBe(baseline.days.find((day) => day.date === '2026-08-26')!.earnedExp);
    expect(updated.weeks[0].weeklyEarnedExp - baseline.weeks[0].weeklyEarnedExp).toBeCloseTo(sourceExperience(excludedDungeon), 2);
  });
  ...
  it('keeps a rechecked first-week task inside the end-of-day baseline', () => {
    ...
    const recheckedRecords = records.map((record) => record.date === scheduleDate
    ...
    expect(updated.days.find((day) => day.date === '2026-08-26')!.end).toEqual(baseline.days.find((day) => day.date === '2026-08-26')!.end);
  ...
  it('settles a newly claimed weekly reward outside every daily experience row', () => {
    ...
    const records = createDefaultRecords(defaultSettings, defaultSources).map((record) => weekKey(record.date) === weekKey(defaultSettings.startDate)
      ? {
        ...record,
        completedSourceIds: record.completedSourceIds.filter((id) => id !== weeklyReward.id),
        baselineIncludedSourceIds: record.baselineIncludedSourceIds?.filter((id) => id !== weeklyReward.id),
      }
      : record);
    const claimedRecords = records.map((record) => record.date === defaultSettings.startDate
    ...
    expect(afterClaim.days.find((day) => day.date === '2026-08-26')!.earnedExp).toBe(beforeClaim.days.find((day) => day.date === '2026-08-26')!.earnedExp);
    expect(afterClaim.weeks[0].weeklyEarnedExp - beforeClaim.weeks[0].weeklyEarnedExp).toBeCloseTo(sourceExperience(weeklyReward), 2);
  ...
  it('does not add a weekly reward again after its claim day is calibrated', () => {
    ...
    const claimedRecords = baseRecords.map((record) => record.date === claimDate
      ? { ...record, completedSourceIds: [...record.completedSourceIds, weeklyReward.id] }
      : record);
  ...
  it('does not subtract a historical first-week task from the current baseline', () => {
    ...
    const baseline = projectTracker(defaultSettings, defaultSources, records);
    const weeklyTask = defaultSources.find((source) => source.id === 'weekly-reward-1')!;
    const withoutWeeklyTask = records.map((record) => weekKey(record.date) === weekKey(defaultSettings.startDate)
